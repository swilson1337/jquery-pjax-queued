...
(function($) {
	...
	function processPjaxQueue() {
		if (queue.length > 0 && !queue[0].started) {
			queue[0].started = true;

			realPjax(queue[0].options);
		}

		resetPjaxQueueTimeout();
	}

	var queueTimeout;

	function resetPjaxQueueTimeout() {
		clearTimeout(queueTimeout);

		queueTimeout = setTimeout(processPjaxQueue, 10);
	}
